...
class HarmonyWebpackModule {
    ...
    getScriptRule() {
        ...
        if (compile.exclude && compile.exclude.length) {
            rule.exclude = [
                ...compile.exclude,
                filename => /css-loader/.test(filename) || (helper_1.REG_NODE_MODULES.test(filename) && !(/taro/.test(filename)))
            ];
        }
            ...
            rule.exclude = [filename => /css-loader/.test(filename) || (helper_1.REG_NODE_MODULES.test(filename) && !(/taro/.test(filename)))];
        }
